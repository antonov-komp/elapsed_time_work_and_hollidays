...
**Это ЭТАП 2 из 6 последовательных этапов задачи TASK-005.**

### Порядок выполнения:
1. ✅ **Этап 1 должен быть завершён** ([TASK-005-01](TASK-005-01-edit-day-modal.md))
2. ✅ **Начните с этого этапа** (TASK-005-02)
3. После завершения → переходите к **Этапу 3** ([TASK-005-03](TASK-005-03-store.md))
4. **НЕ пропускайте этапы!** Выполняйте строго по порядку.

### Что нужно сделать:
- Создать компонент `FillWeekButton.vue` для заполнения недели
- Реализовать логику заполнения рабочих дней
- Протестировать компонент

### Требования:
- TASK-005-01: Компонент модального окна (может быть не завершён, но не критично)
- TASK-003-04: Утилиты для работы с датами (должны быть готовы)
...
- **2025-12-12 12:50 (UTC+3, Брест):** Реализован компонент FillWeekButton.vue:
  - ✅ Создана структура компонента с template, script и style
  - ✅ Реализовано определение текущей недели с учётом текущего месяца
  - ✅ Реализовано заполнение рабочих дней (8 часов)
  - ✅ Реализовано исключение выходных дней
  - ✅ Реализовано исключение праздничных дней
  - ✅ Реализовано подтверждение действия перед заполнением
  - ✅ Реализована обработка сохранения (emit события)
  - ✅ Добавлены стили, анимации и состояние загрузки
  - ✅ Компонент адаптирован под существующие утилиты dateHelpers
